import logging
from PySide6.QtCore import Qt, Signal, Slot, QMargins, QEvent, QPoint, QTimer, SLOT
...
class PixmapPopUp(QWidget):
    ...
    def event(self, e: QEvent) -> bool:
        ...
        elif e.type() == QEvent.Leave or e.type() == QEvent.Close:
            if self._is_entered:
...
class PixmapSizeOverlay(QWidget):
    ...
    def paintEvent(self, ev: QPaintEvent) -> None:
        ...
        if self._position == PixmapSizeOverlay.CENTER:
            ...
            pos = (
                parent.rect().bottomLeft() - self.rect().bottomLeft()
                if parent
                else QPoint()
            )
        self.move(pos)
